...
| Round-trip golden tests (curated corpus) | ✅ | N8f — `test/linx/nft/fixtures/*.nft` — 8 hand-written fixtures (empty table, minimal stateful, NAT port-forward, multi-chain dispatch, named set, Nerves appliance, IPv6 filter, container guard). Each fixture asserted in two directions: `parse → format → parse` structural equality + `format → parse → format` byte-identical output stability. Adding a fixture is just dropping a `.nft` file in the directory. Aspirational corpus (real-world configs from Debian / Arch / Kubernetes / firewalld / nftables wiki) deferred until the long-tail features land |

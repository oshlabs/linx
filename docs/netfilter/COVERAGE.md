...
| Round-trip golden tests (curated corpus) | ✅ | N8f, extended N8k — `test/linx/nft/fixtures/*.nft` — 9 fixtures (empty table, minimal stateful, NAT port-forward, multi-chain dispatch, named set, Nerves appliance, IPv6 filter, container guard, real-world home-router shape). Each fixture asserted in three directions: parses, `parse → format → parse` structural equality, `format → parse → format` byte-identical output stability. Adding a fixture is just dropping a `.nft` file in the directory. Aspirational corpus (real-world configs from Debian / Arch / Kubernetes / firewalld / nftables wiki) deferred until the remaining long-tail features land (limit, meta-set, named objects, flowtables, concat keys, includes, defines, NPTv6) |
| Bare `iifname` / `oifname` / `iif` / `oif` shorthand (no `meta` prefix) | ✅ | N8g — parser accepts both `iifname "lo"` and `meta iifname "lo"` |
| `ct state {NAMES}` / `ct state N1,N2` inline-set matches | ✅ | N8h — compiler emits the kernel-correct `bitwise(mask) + cmp_neq_0` pattern. Inverts to `cmp_eq_0` for `!=`. Both brace and comma-no-brace syntactic forms produce identical wire-shape. Formatter inverse |
| `icmpv6 type` / `icmpv6 code` matches | ✅ | N8i — `Expr.payload(:icmpv6_type / :icmpv6_code)` aliases. Symbolic ICMPv6 type names (`echo-request`, `nd-router-solicit`, etc.) resolved in `parse_int_keyword/1`. Inline sets of symbolic names resolve each element to its integer code |
| `flush ruleset` top-level directive | ✅ | N8j — parser accepts it; compiler treats as noop (Ruleset.new() is already empty); formatter doesn't re-emit it (value type doesn't track "was flushed at source") |
...
| `mix format` plugin (`Linx.NFT.Formatter`) | ✅ | N9 — `Linx.NFT.Formatter` implements `Mix.Tasks.Format`: `features/1` advertises `[sigils: [:NFT], extensions: [".nft"]]`; `format/2` parses + reflows static sigil bodies and `.nft` files through `format/1`. Interpolation-bearing sigil bodies pass through unchanged (AST-aware re-emission deferred). Idempotent — same invariant N8f's output-stability assertion guarantees |

...
- **`attach(:group_leader, c)`** — pumps through `Process.group_leader/0` via Erlang's I/O protocol. Works over SSH, `:remsh`, and locally as a universal mode. Polled resize (~250 ms). The headline mode for SSH-into-a-Nerves-device workflows.
...
- **Live resize.** `:controlling` propagates `SIGWINCH` via a `:gen_event` handler on OTP's `:erl_signal_server` for sub-millisecond redraws. `:group_leader` polls geometry on a 250 ms cadence (no SSH-side SIGWINCH equivalent surfaced through `:io`). The trade-off against an event-driven `:erlang.trace/3` hook is documented inline on `@winsize_poll_ms` in `lib/linx/tty.ex` — TL;DR: polling is cheaper, simpler, and human-imperceptible at the chosen cadence.

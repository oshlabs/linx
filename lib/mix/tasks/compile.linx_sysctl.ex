defmodule Mix.Tasks.Compile.LinxSysctl do
  # Mix compiler for the `linx_sysctl` NIF.
  #
  # Compiles `c_src/linx_sysctl.c` into a shared library in the
  # application's `priv/` directory, where `Linx.Sysctl.Native` loads
  ...
  #
  # Sibling to `compile.linx_mount` -- same shape, same conventions.
  #
  ...
  def run(_args) do
    ...
    cond do
      not File.exists?(source) ->
        Mix.raise("linx_sysctl: missing C source #{@source}")
  ...
  defp compile(source, output) do
    ...
    case System.cmd(cc, args, stderr_to_stdout: true) do
      ...
      {output, status} ->
        Mix.raise("linx_sysctl: #{cc} failed (exit #{status})\n#{output}")
    end
  end
